...
# ══════════════════════════════════════════════════════════════════════════════
# 2. GEAR RACE — top 5 cameras, cumulative by month Jan 2021 – Dec 2024
# ══════════════════════════════════════════════════════════════════════════════
top5 = df4["Model_norm"].value_counts().head(5).index.tolist()
print(f"\nTop 5 cameras: {top5}")

months = [(y, m) for y in range(2021, 2025) for m in range(1, 13)]
month_idx = {(y, m): i for i, (y, m) in enumerate(months)}

gear_df = df4[df4["Model_norm"].isin(top5)].copy()
gear_df["month_i"] = gear_df.apply(lambda r: month_idx.get((r["Year"], r["Month"]), None), axis=1)
gear_df = gear_df.dropna(subset=["month_i"])
gear_df["month_i"] = gear_df["month_i"].astype(int)

monthly = {c: [0]*48 for c in top5}
...
for cam in top5:
    cum = 0
    cumulative[cam] = []
    for v in monthly[cam]:
        cum += v
        cumulative[cam].append(cum)

print("\nGear Race totals at Dec 2024:")
for cam in top5:
    print(f"  {cam}: {cumulative[cam][-1]}")

# ══════════════════════════════════════════════════════════════════════════════
# Write viz_data.js
# ══════════════════════════════════════════════════════════════════════════════
cameras_js = json.dumps(top5)
...
const GEAR_CUMULATIVE = {json.dumps(cumulative, indent=2)};
"""
